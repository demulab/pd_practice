...
ER ev3_lcd_set_font(lcdfont_t font) {
  sil_wrb_mem((uint32_t *)EV3_LCD_ADDR_FONT, (int)font);
...
// TODO: #41 temporary comment out
  ...
  //int result = (str[0] << 24 | str[1] << 16 | str[2] << 8 | str[3]);
  //sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_STRING, result);
  //same typing situation as motor_dri.c
  int xInt = (int) x;
  int yInt = (int) y;

  
  sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_X, x);
  sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_Y, y);
  
  for(int i = 0; i < len; i++){
	  if (len >= 32)
		  break;
	  sil_wrb_mem((uint32_t *) (EV3_LCD_ADDR_STRING + i), str[i]);
  }
  if (len < 32) {
	  for (int i = 0; i < 32 - len; i++) {
		  sil_wrb_mem((uint32_t*)(EV3_LCD_ADDR_STRING + i + len), '\0');
	  }
  }
...
//	bitmap_bitblt(NULL, 0, 0, lcd_screen, x, y, w, h, color == EV3_LCD_BLACK ? ROP_SET : ROP_CLEAR);
  sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_RECT_X, x);
  sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_RECT_Y, y);
  sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_RECT_W, w);
  sil_wrw_mem((uint32_t *)EV3_LCD_ADDR_RECT_H, h);
  sil_wrb_mem((uint32_t *)EV3_LCD_ADDR_RECT_C, (int)color);
  return E_OK;
}
