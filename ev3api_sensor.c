...
// このAPIは箱庭では未実装なので実装した。 demu
// 超音波センサの値が250のときは障害物がないことを利用している。
...
//	lazy_initialize();
	...
	// TODO: TEST THIS API!
	// bool_t val;
	// uart_sensor_fetch_data(port, US_LISTEN, &val, sizeof(val)); // demu


	int16_t val = COLOR_NONE;

#if 0
    val = ev3_uart_sensor_get_short(port) / 10;
    if (val == 250) return false;
    else return true;

#endif
    uart_sensor_fetch_data(port, US_DIST_CM, &val, sizeof(val));
    if (val/10 == 250) return false;
    else return true;
